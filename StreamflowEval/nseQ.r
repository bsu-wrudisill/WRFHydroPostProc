...
merged$station <- as.integer(merged$station)
#merged$station <- as.integer(merged$station)

# ---- NSE for station 1 ----------# 
obs   = subset(merged, merged$run=="Observation" & merged$station==1, select= c(q_cms, dateTime))
model = subset(merged, merged$run=="Gridded Baseline" & merged$station==1, select= c(q_cms))


aggregate(cbind(q_cms,dateTime) ~ Hour, obs, mean)


#Nse(model, obs, nullModel = mean(obs, na.rm = na.rm), na.rm = TRUE)
